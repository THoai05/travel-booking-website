...
export function TablesSectionKPI() {
  ...
  const exportExcel = () => {
    ...
    Object.keys(worksheet).forEach((key) => {
      if (key.startsWith("D") && key !== "D1") { // Cột Revenue
        const cell = worksheet[key];
        if (cell.v > 0) {
          cell.s = {
            font: { bold: true, color: { rgb: "006400" } }, // dark green
            fill: { fgColor: { rgb: "E6FFE6" } }, // light green
          };
        }
      }
      // Trạng thái cột E -> I
      const statusCols = ["E", "F", "G", "H", "I"];
      statusCols.forEach((col) => {
        if (key.startsWith(col) && key !== `${col}1`) {
          const val = worksheet[key].v;
          if (val > 0) {
            let color = "D9D9D9"; // default gray
            switch (col) {
              case "E": color = "FFF2CC"; break; // pending - yellow
              case "F": color = "D9EAD3"; break; // confirmed - green
              case "G": color = "F4CCCC"; break; // cancelled - red
              case "H": color = "CFE2F3"; break; // completed - blue
              case "I": color = "E0E0E0"; break; // expired - gray
            }
            worksheet[key].s = {
              font: { bold: true },
