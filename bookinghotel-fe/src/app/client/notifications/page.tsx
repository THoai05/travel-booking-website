        ...
        <div className="min-h-screen flex flex-col items-center bg-gray-50 p-4 sm:p-6" onClick={() => toast.dismiss()}>
            ...
            <div className="flex w-full gap-4 mb-6">
                ...
                >
                    ...
                    {selectedIds.length === paginatedNotifications.length ? "Bỏ chọn tất cả" : "Chọn tất cả"}
                </button>
            ...
            <div className="w-full max-w-2xl flex flex-col gap-3">
              ...
              ${selectedIds.includes(n.id) ? "border-blue-500 ring-1 ring-blue-200" : "hover:border-blue-400"}`}
                        onClick={() => n.url ? window.open(n.url, "_blank") : openDetail(n.id)}
                    >
                        <div className="flex gap-3 items-start">
                            ...
                            <div className="flex flex-col">
                                <h3 className="text-md font-semibold text-gray-800 line-clamp-2">{n.title || "Tiêu đề"}</h3>
                                <p className="text-xs text-gray-500 mb-1 capitalize">{n.type}</p>
                                <p className="text-gray-700 text-sm">{n.message.length > 100 ? n.message.substring(0, 100) + "..." : n.message}</p>
                            </div>
                        </div>
                        <div className="flex flex-col items-end gap-2">
                            <span className="text-xs text-gray-400">{new Date(n.createdAt).toLocaleString("vi-VN", { hour12: false })}</span>
                            <input type="checkbox" checked={selectedIds.includes(n.id)} onChange={(e) => { e.stopPropagation(); toggleSelect(n.id); }} className="w-4 h-4 accent-blue-600 cursor-pointer" />
                        </div>
                    </div>
                ))}
            ...
            {totalPages > 1 && (
                <div className="flex flex-wrap justify-center gap-2 mt-4">
                    <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="px-3 py-1 border rounded bg-gray-200 disabled:opacity-50">Previous</button>
                    {getPaginationNumbers().map((num, idx) => (
                        <button key={idx} onClick={() => typeof num === "number" && setCurrentPage(num)} disabled={num === "..."} className={`px-3 py-1 border rounded ${num === currentPage ? "bg-blue-500 text-white" : "bg-gray-200"}`}>{num}</button>
                    ))}
                    <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="px-3 py-1 border rounded bg-gray-200 disabled:opacity-50">Next</button>
                </div>
            )}
            ...
            {detailId && detailData && (
                <div className="fixed inset-0 bg-black/40 flex justify-center items-start p-4 overflow-auto z-50">
                    <div className="bg-white rounded-[5px] p-6 w-full max-w-md relative" onClick={(e) => e.stopPropagation()}>
                        <button onClick={closeDetail} className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 font-bold text-xl">×</button>
                        {detailLoading ? (
                            ...
                            <>
                                ...
                                <p className="text-sm text-gray-500 mb-2">Loại thông báo: <span className="font-medium text-gray-700 capitalize">{detailData.type}</span></p>
                                <p className="text-gray-700 mb-4">{detailData.message}</p>
                                {detailData.url && (
                                    <a href={detailData.url} target="_blank" className="text-blue-500 underline mb-4 block">Xem chi tiết</a>
                                )}
                                <span className="text-sm text-gray-500">{new Date(detailData.createdAt).toLocaleString("vi-VN", { hour12: false })}</span>
                            </>
                        )}
                    </div>
                </div>
